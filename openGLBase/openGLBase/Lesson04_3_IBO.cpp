...
unsigned int Lesson04_3_IBO::createTexture(int w, int h, const void* data)
{
	unsigned texId;
...
unsigned int Lesson04_3_IBO::createTextureFromImage(const char* fileName)
{
	// 获取图片格式
...
void Lesson04_3_IBO::createVbo()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(verCude3), verCude3, GL_STATIC_DRAW);
	//glBufferSubData(); //动态更新数据
	// 恢复状态，不然会对后续绘制产生影响
	glBindBuffer(GL_ARRAY_BUFFER, 0);
}

void Lesson04_3_IBO::createIbo()
{
	glGenBuffers(1, &m_unIbo);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_unIbo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indexVer), indexVer, GL_STATIC_DRAW);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
}

void Lesson04_3_IBO::drawCude()
{
	glBindBuffer(GL_ARRAY_BUFFER, m_unVbo);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_unIbo);
